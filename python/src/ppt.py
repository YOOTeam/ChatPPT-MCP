...
# 设置ppt主题色
@mcp.tool()
async def ppt_set_color(ppt_id: str = Field(description="PPT-ID"), 
                        color:str = Field(description="PPT-color")) -> str:
    """
    Name:
        更改设置PPT主题色。
    Description:
    ...
    Args:
        ppt_id: PPT-ID
        color：PPT-color,设置的主题色，可以支持颜色空间名称如"紫色","红色","橙色","黄色","绿色","青色","蓝色","粉色",也可以支持#xxxxxh或者RGB颜色值。
    Returns:
        新的PPT-ID
    """
...
# 设置ppt字体
@mcp.tool()
async def ppt_set_fontName(ppt_id: str = Field(description="PPT-ID"),
                           font_name: str = Field(description="font_name")) -> str:
    """
    Name:
        设置更改字体。
    Description:
    ...
    Args:
        ppt_id: PPT-ID
        font_name：字体名，如黑体、宋体、仿宋、幼圆、楷体、隶书
    Returns:
        新的PPT-ID
    """
...
async def ppt_set_anim(ppt_id: str = Field(description="PPT-ID"),
                       setAnim:str= Field(default="1", description="设置动画还是取消动画")) -> str:
    """
    Name:
        更改设置动画。
    Description:
    ...
    Args:
        ppt_id: PPT-ID
        setAnim：是否设置，默认为“1”表示设置动画，“0”标识取消动画。
    Returns:
        新的PPT-ID
    """
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                url,
                data={'id': ppt_id,"transition":setAnim},
                headers={'Authorization': 'Bearer ' + API_PPT_KEY},
                timeout=60
            )
            response.raise_for_status()
...
async def ppt_add_slides(ppt_id: str = Field(description="PPT-ID"), 
    ...
    Args:
        ppt_id: PPT-ID，必须参数。
        ppt_text: slide_text，用户指定页面的内容主题文本，必须参数。
        ppt_text: slide_type，指定生成页面类型，可以是“封面页，目录页，章节页，内容页，致谢页”，默认为内容页。
    Returns:
        新的PPT-ID
    """
...
# 生成PPT大纲
@mcp.tool()
async def ppt_create_outlineText(
...
# 生成PPT模板预览图
@mcp.tool()
async def ppt_create_template_coverImage(
    ppt_text: str = Field(description="PPT-themeText"),
    ppt_color: str = Field(default_factory=["紫色","红色","橙色","黄色","绿色","青色","蓝色","粉色","灰色"], description="tempalte-color"),
    ppt_style: str = Field(default_factory=["科技风","商务风","小清新","极简风","中国风","可爱卡通"], description="tempalte-style"),
    ppt_num: int = Field(default=4, description="Template-num")) -> str:
    """
    Name:
    ...
    Args:
        ppt_text: PPT-themeText，用户输入主题文本，必须参数。
        ppt_color：Tempalte-color，指定生成的模板风格，可以为空，表示随机；也可以从"科技风","商务风","小清新","极简风","中国风","可爱卡通"进行执行，可选参数。
        ppt_style: Tempalte-style,指定生成模板的颜色，可以为空，表示随机；也可以从"紫色","红色","橙色","黄色","绿色","青色","蓝色","粉色","灰色"进行指定，可选参数。
        ppt_num：Template-num，指定生成模板数量，默认为4
    Returns:
        新的PPT-ID
    """
...
# 替换/更换指定模板
@mcp.tool()
async def ppt_replace_user_select_template(ppt_id: str = Field(description="PPT-ID"),
                                           cover_id: str = Field(description="cover_id")) -> str:
    """
    Name:
        更改替换为用户指定模板。
    Description:
    ...
    Args:
        ppt_id: PPT-ID
        cover_id：cover_id，用户指定的模板id，需要通过ppt_create_template_coverImage 进行生成。
    Returns:
        新的PPT-ID
    """
